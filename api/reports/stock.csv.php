...
$rows = $pdo->query("
    SELECT i.sku, i.name, l.name AS location_name, COALESCE(s.qty_on_hand, 0) AS qty_on_hand,
           i.unit_of_measure, i.reorder_point, i.unit_cost
    FROM items i
...
fputcsv($out, ['SKU', 'Name', 'Location', 'Qty On Hand', 'Unit', 'Reorder Point', 'Unit Cost', 'Value'], ',', '"', '\\');
foreach ($rows as $r) {
    fputcsv($out, [
        $r['sku'], $r['name'], $r['location_name'], $r['qty_on_hand'], $r['unit_of_measure'],
        $r['reorder_point'], $r['unit_cost'], round($r['unit_cost'] * $r['qty_on_hand'], 2),
    ], ',', '"', '\\');
}
fclose($out);
