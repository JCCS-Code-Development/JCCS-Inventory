...
$auth = requireAuth(); // all three roles can take stock — this is the basic self-service action
...
try {
    $stmt = $pdo->prepare('SELECT qty_on_hand FROM item_stock WHERE item_id = ? AND location_id = ? FOR UPDATE');
    $stmt->execute([$itemId, $locationId]);
    $current = $stmt->fetchColumn();
    $currentQty = $current === false ? 0.0 : (float)$current;

    if ($qty > $currentQty) {
        $pdo->rollBack();
        http_response_code(422);
        exit(json_encode(['error' => "Only $currentQty available at this location"]));
    }
    $qtyAfter = $currentQty - $qty;

    $pdo->prepare(
        'INSERT INTO item_stock (item_id, location_id, qty_on_hand) VALUES (?, ?, ?)
         ON DUPLICATE KEY UPDATE qty_on_hand = VALUES(qty_on_hand)'
    )->execute([$itemId, $locationId, $qtyAfter]);
    ...
    $pdo->commit();
    echo json_encode(['message' => 'Checked out', 'qty_after' => $qtyAfter]);
} catch (Throwable $e) {
    $pdo->rollBack();
    throw $e;
}
